...
module.exports = {
    name: 'forward',
    description: 'Forward the current song by specified seconds',
    aliases: ['fw'],
    category: 'music',
    premium: true,
    player: true,
    ...
    async execute(client, message, args, prefix) {
        ...
        const newPosition = Math.min(player.queue.current.duration, player.position + (seconds * 1000));
        player.seek(newPosition);

        const embed = new EmbedBuilder()
            .setColor(config.embedColor)
            .setDescription(`${emojis.success} Forwarded **${seconds}** seconds in [${player.queue.current.title}](${player.queue.current.uri})`);
